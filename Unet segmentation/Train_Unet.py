...
MODEL_DIR = 'unet_model/again/'  # 模型保存路径
BATCH_SIZE = 16
...
def image_generator(all_files: List[Tuple[str, str]],
    ...
    while True:
        # 1) 取 batch
        if train_set:
            batch_files = (random.sample(pos_list, half) +
                           random.sample(neg_list, batch_size-half))
            random.shuffle(batch_files)
        else:
              # 对验证集也做 1:1 随机平衡采样
                batch_files = (random.sample(pos_list, half)+random.sample(neg_list, batch_size - half))
                random.shuffle(batch_files)
