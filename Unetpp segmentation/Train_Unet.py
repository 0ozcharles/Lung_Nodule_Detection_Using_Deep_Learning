...
os.makedirs('./temp_dir/chapter5/', exist_ok=True)
# 数据增强
...
def get_train_holdout_files():
    def _read_one_csv(csv_path: str) -> List[Tuple[str, str]]:
        """把单个 csv 里 <img_path>,<mask_path> 读成 list[(img, mask), …]"""
        pairs = []
        with open(csv_path, newline='') as f:
            for row in csv.reader(f):
                ...
                # ① 如果真的只有一列，就按原来的规则推断 mask 路径
                if len(row) == 1 or not row[1].strip():
                    img  = row[0].strip()
                    mask = img.replace("/images/", "/masks/").replace("_img.png", "_mask.png")
                else:
                    # ② 正常情况：两列都写在 csv 里
                    img, mask = row[0].strip(), row[1].strip()

                # ③ 文件必须都存在，否则直接跳过并给个提示
                if not (os.path.isfile(img) and os.path.isfile(mask)):
                    print(f"⚠️  跳过缺失文件: {img}  or  {mask}")
                    continue

                pairs.append((img, mask))
        return pairs

    train_res   = _read_one_csv(TRAIN_LIST)
    holdout_res = _read_one_csv(VAL_LIST)

    print(f"Train count:  {len(train_res)} , holdout count:  {len(holdout_res)}")
...
def dice_coef(y_true, y_pred, smooth=1e-5):
    """标准 soft‑Dice，smooth=1 避免分母 0，又不会淹没梯度"""
    y_true_f = K.flatten(y_true)
    y_pred_f = K.flatten(y_pred)
...
#         # 每种模型保存到不同路径
#         self.dump_dir = f'./temp_dir/vis/{model_type}/'
...
#             cv2.imwrite(self.dump_dir + "img_{0:03d}_{1:02d}_i.png".format(epoch, i), x)
#             cv2.imwrite(self.dump_dir + "img_{0:03d}_{1:02d}_o.png".format(epoch, i), y)
#             cv2.imwrite(self.dump_dir + "img_{0:03d}_{1:02d}_p.png".format(epoch, i), y_pred)
class DumpPredictions(Callback):
    def __init__(self, dump_filelist: List[Tuple[str, str]], model_type):
        super(DumpPredictions, self).__init__()
        self.dump_filelist = dump_filelist
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(10):
            ...
            # 如果模型有多个输出（如Unet++深度监督），只取最后一个输出
            if isinstance(y_pred, list):
                y_pred = y_pred[-1]

            # squeeze 去除 batch 维和通道维
            x = np.squeeze(x) * 255.
            y = np.squeeze(y) * 255.
            ...
            cv2.imwrite(f"{self.dump_dir}img_{epoch:03d}_{i:02d}_i.png", x)
            cv2.imwrite(f"{self.dump_dir}img_{epoch:03d}_{i:02d}_o.png", y)
            cv2.imwrite(f"{self.dump_dir}img_{epoch:03d}_{i:02d}_p.png", y_pred)
...
def image_generator(all_files: List[Tuple[str, str]],
                    batch_size: int,
                    train_set: bool):
    """
    · train_set = True  →  每个 batch 随机抽样，并保证 “正 : 负 = 1 : 1”
    · train_set = False →  顺序遍历，确保验证集所有切片都被评估到
    """
    if train_set:
        assert batch_size % 2 == 0, "batch_size 必须是偶数以保持 1:1 平衡"
    half = batch_size // 2

    # 1) 先拆正 / 负（只做一次）
    pos_list = [p for p in all_files
                if cv2.imread(p[1], cv2.IMREAD_GRAYSCALE).max() > 0]
    neg_list = [p for p in all_files
                if cv2.imread(p[1], cv2.IMREAD_GRAYSCALE).max() == 0]

    idx, n = 0, len(all_files)          # 用于验证集顺序遍历
    global ELASTIC_INDICES

    while True:
        # 2) 组成本轮 batch 路径列表
        if train_set:                   # —— 训练集：1:1 随机采样 ——
            batch_files = (random.choices(pos_list, k=half) +
                           random.choices(neg_list, k=half))
        ...
        img_list, mask_list = [], []
        ELASTIC_INDICES = None          # 防止 elastic_transform 缓存跨样本共享

        # 3) 逐张读取 & 可选增强
        for img_path, msk_path in batch_files:
            ...
            if train_set:
                ...
                if random.random() < 0.3:
                    img  = elastic_transform(img , 128, 15)
                    mask = elastic_transform(mask, 128, 15)

                aug = [img, mask]
                # 2) 随机旋转 ±15°（50%）
                aug = random_rotate_img(aug, 0.5, -15, 15)
                # 3) 水平/垂直翻转（各 50%）
                aug = random_flip_img(aug, 0.5, 0.5)
                # 4) 亮度/对比度扰动（50%）
                if random.random() < 0.5:
                    aug = random_distort(aug)
                # 5) 平移扰动 ±15 px（30%）
                if random.random() < 0.3:
                    aug = random_translate_img(aug, XYRange(-15, 15, -15, 15, 0.3))
                # 拆包回 img, mask
                img, mask = aug
            # --------------------------------

            # 4) 归一化到 [0,1] 并加通道维
            img  = (img .astype(np.float32) / 255.)[..., None]   # (H,W,1)
            mask = (mask.astype(np.float32) / 255.)[..., None]   # (H,W,1)

            img_list.append(img)
            mask_list.append(mask)

        # 5) 组 batch
        yield np.stack(img_list, 0), np.stack(mask_list, 0)




from keras.optimizers import Adam       # 追加 import

# 实现unet的网络结构，并加载预训练好的权重
def get_unet(learn_rate=0.001) -> Model:
    ...
    #               loss=dice_loss,
    #               metrics=[dice_coef])
    model.compile(optimizer=Adam(learning_rate=1e-3),  # 初始 lr 提到 3e‑3
                  loss=dice_loss,
                  metrics=[dice_coef])
    # Adam(lr=1e-5)
...
def train_model(model_type, continue_from=None):
    ...
    # hist = model.fit(train_gen, steps_per_epoch=200, epochs=80,
    #                                      validation_data = holdout_gen, verbose = 1,
    #                   callbacks = [checkpoint1, tensorbd,dumper], validation_steps = 10)
    # —— 计算验证集一共要跑多少步（最后一步可能不足 batch_size）

    steps_per_epoch = len(train_files) // batch_size  # 遍历一遍训练集
...
if __name__ == "__main__":
    TRAIN_LIST = './train_uid_list.csv'
    VAL_LIST = './val_uid_list.csv'
    # train_model(model_type='u-net', continue_from='./model/again/u-net_28-0.37.hd5')
    train_model(model_type='u-net', continue_from=None)
