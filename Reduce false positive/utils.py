...
st.train=False
import pandas as pd
def filter_csv_by_raw_and_test(csv_path, raw_dir, test_dir):
    df = pd.read_csv(csv_path)
    available_raw  = {f[:-4] for f in os.listdir(raw_dir)  if f.endswith(".mhd")}
    available_test = {f[:-4] for f in os.listdir(test_dir) if f.endswith(".mhd")}
    print(f"[filter] raw 卷数: {len(available_raw)}, test 卷数: {len(available_test)}")
    df_train = df[df["seriesuid"].isin(available_raw)].reset_index(drop=True)
    df_test  = df[df["seriesuid"].isin(available_test)].reset_index(drop=True)
    print(f"[filter] 训练候选点: {len(df_train)}, 测试候选点: {len(df_test)}")
...
def load_raw_data():
    """
    Load raw .mhd for train or test, build patch_dat, patch_lbl.
    In train mode, save to disk; in test mode, return arrays.
    """
    # paths
    csv_path = os.path.join(st.data_path, "candidates_V2.csv")
    raw_dir  = os.path.join(st.data_path, "raw")
    test_dir = os.path.join(st.data_path, "test")

    # split
    df_train, df_test = filter_csv_by_raw_and_test(csv_path, raw_dir, test_dir)
    df_used = df_train if st.train else df_test

    # truncate for train, full for test
    if st.train:
        unique_sids = df_used["seriesuid"].unique()[:PATIENT_NUM]
        df = df_used[df_used["seriesuid"].isin(unique_sids)].reset_index(drop=True)
    else:
        df = df_used.copy()
    ...
    for _, row in df.iterrows():
        sid = row["seriesuid"]
        if sid not in fn_list:
            fn_list[sid] = len(fn_list)
        candidates.append([
            fn_list[sid],
            int(row["class"]),
            float(row["coordZ"]),
    ...
    def normalizePlanes(x):
        x = (x + 1000.) / 1400.
        return np.clip(x, 0, 1)

    def worldToVoxelCoord(wc, origin, spacing):
        return np.round(np.abs(wc - origin) / spacing).astype(int)

    # allocate
    patch_dat = np.zeros((len(candidates), slice_z, slice_y, slice_x), dtype=np.float32)
    patch_lbl = np.zeros((len(candidates), 8), dtype=np.float32)
    cnt = 0

    # process each series
    for sid, sid_idx in fn_list.items():
        base_dir = raw_dir if st.train else test_dir
        mhd_path = os.path.join(base_dir, f"{sid}.mhd")
        if not os.path.isfile(mhd_path):
            raise FileNotFoundError(f"在 {base_dir} 中找不到 {sid}.mhd")
        vol, origin, spacing = load_itk_image(mhd_path)
        vol = normalizePlanes(vol)

        mask = (candidates[:, 0] == sid_idx)
        pts  = candidates[mask, 1:]
        vox  = worldToVoxelCoord(pts[:,1:], origin, spacing)
        for (lbl, oZ, oY, oX), (Z, Y, X) in zip(pts, vox):
            d = vol[
                max(Z-slice_z//2,0):Z+slice_z//2,
                max(Y-slice_y//2,0):Y+slice_y//2,
                max(X-slice_x//2,0):X+slice_x//2
            ]
            if d.shape != (slice_z, slice_y, slice_x):
                padZ = (max(slice_z//2-Z,0), slice_z//2-min(vol.shape[0]-Z,slice_z//2))
                padY = (max(slice_y//2-Y,0), slice_y//2-min(vol.shape[1]-Y,slice_y//2))
                padX = (max(slice_x//2-X,0), slice_x//2-min(vol.shape[2]-X,slice_x//2))
                d = np.pad(d, (padZ, padY, padX), mode="constant", constant_values=0.)
            patch_dat[cnt] = d
            patch_lbl[cnt] = [lbl, sid_idx, Z, Y, X, oZ, oY, oX]
            cnt += 1

    # save or return
    if st.train:
        os.makedirs(save_data_path, exist_ok=True)
        np.save(os.path.join(save_data_path, "patch_dat.npy"), patch_dat)
        np.save(os.path.join(save_data_path, "patch_lbl.npy"), patch_lbl)
        print(f"[load_raw_data] (TRAIN) saved {cnt} patches.")
        return
    else:
        print(f"[load_raw_data] (TEST) created {cnt} patches in memory.")
        return patch_dat, patch_lbl
...
def load_fold(fold_num=-1):
    # TEST mode: load and resize in memory (no disk write)
    if not st.train:
        # Generate raw patches and labels in memory
        patch_dat, patch_lbl = load_raw_data()
        rd = patch_dat[:, 1:-1, 1:-1, 1:-1]

        # Bottom scale (center crop)
        btm = rd[:, 10:-10, 10:-10, 10:-10]
        # Middle scale zoom
        mid = rd[:,  8:-8,   5:-5,   5:-5]
        # Top scale zoom uses full rd
        td  = rd

        N = rd.shape[0]
        # Allocate resized array: (num_samples, 3 streams, H=20, W=20, D=6)
        test_dat = np.zeros((N, 3, 20, 20, 6), dtype=np.float32)
        for i in range(N):
            # Each patch is (D,Z,H,W) => zoom and then transpose to (H,W,D)
            z0 = btm[i]  # shape (6,20,20)
            z1 = nd.zoom(mid[i], zoom=(6/10, 20/30, 20/30), mode='nearest')  # (6,20,20)
            z2 = nd.zoom(td[i],  zoom=(6/(slice_z-2), 20/(slice_y-2), 20/(slice_x-2)), mode='nearest')  # (6,20,20)

            # Transpose to (20,20,6)
            test_dat[i, 0] = np.transpose(z0, (1, 2, 0))
            test_dat[i, 1] = np.transpose(z1, (1, 2, 0))
            test_dat[i, 2] = np.transpose(z2, (1, 2, 0))

        # Extract labels
        test_lbl = patch_lbl[..., 0, None]
        return None, None, test_dat, test_lbl

    # TRAIN mode: original split_fold + CV logic
