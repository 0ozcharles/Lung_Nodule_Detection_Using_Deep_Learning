...
tf.disable_eager_execution()   # 等价于原来的 tf.compat.v1.disable_eager_execution()
import model
...
def test_proposed():
    ...
    # —— 单次测试 subset3 ——
    _, _, tst_dat, tst_lbl = utils.load_fold()
    print(f"[Test] subset3 patch 数量: {len(tst_dat)}")

    # 构建模型
    placeholders = {
        "bdat": tf.compat.v1.placeholder(shape=(None, 20, 20, 6), dtype=tf.float32, name="bdat_place"),
        "mdat": tf.compat.v1.placeholder(shape=(None, 20, 20, 6), dtype=tf.float32, name="mdat_place"),
        "tdat": tf.compat.v1.placeholder(shape=(None, 20, 20, 6), dtype=tf.float32, name="tdat_place"),
        # 即便测试不用，也必须给个形状对得上的 lbl placeholder
        "lbl": tf.compat.v1.placeholder(shape=(None, 1),
                                        dtype=tf.int64,
                                        name="lbl_place"),  # ← 一定要加上
        "train": st.is_training
    }
    models = model.MGICNN(placeholders)
    models.build_proposed()

    # 一次性按 batch 预测
    all_pm = np.zeros((len(tst_dat),), dtype=np.float32)
    saver = tf.compat.v1.train.Saver()
    with tf.compat.v1.Session() as sess:
        sess.run(tf.compat.v1.global_variables_initializer())
        ckpt = tf.train.latest_checkpoint(st.tst_model_path)
        saver.restore(sess, ckpt)
        for i in range(0, len(tst_dat), st.batch_size):
            batch = tst_dat[i : i + st.batch_size]
            feed = {
                placeholders["bdat"]: batch[:,0],
                placeholders["mdat"]: batch[:,1],
                placeholders["tdat"]: batch[:,2],
                placeholders["train"]: False
            }
            pm = sess.run(models.pred_sig, feed_dict=feed)[:,0]
            all_pm[i : i + len(pm)] = pm

    # 保存预测
    os.makedirs(st.summ_path_root + "pred_result", exist_ok=True)
    np.save(f"{st.summ_path_root}pred_result/{st.model_mode}_{st.multistream_mode}_pm.npy",
            all_pm)
    np.save(f"{st.summ_path_root}pred_result/{st.model_mode}_{st.multistream_mode}_lbl.npy",
            tst_lbl[:, 0])
    print("result saving done!")
